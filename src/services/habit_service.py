...
async def create_habit(payload: HabitCreate, user_id: str) -> HabitOut:
    habit = HabitOut(
        id=str(uuid4()),
        name=payload.name,
        emoji=payload.emoji,
    ...
    await redis_client.redis.set(habit_key(user_id, habit.id), habit.model_dump_json())
    return habit


async def get_all_habits(user_id: str) -> list[HabitOut]:
    keys = await redis_client.redis.keys(habits_pattern(user_id))
    if not keys:
        return []
...
async def get_habit(habit_id: str, user_id: str) -> HabitOut | None:
    data = await redis_client.redis.get(habit_key(user_id, habit_id))
    if not data:
        return None
